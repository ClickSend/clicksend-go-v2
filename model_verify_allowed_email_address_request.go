/*
SMS

You can use the SMS endpoints to send and manage SMS, check the cost of sending SMS, view message history, and more. Integrate them into your system as part of your workflow.  Relevant guides: <a href=\"https://help.clicksend.com/article/h474eseq3a-how-many-characters-can-i-send-in-an-sms\" target=\"_blank\">How many characters can I send in an SMS?</a>, <a href=\"https://help.clicksend.com/article/1hd33n2xns-sms-status-delivery-receipt-expired\" target=\"_blank\">SMS status delivery receipt expired</a>, <a href=\"https://help.clicksend.com/en/articles/42195-unicode-vs-plain-text\" target=\"_blank\">Do you support non-English characters</a>, and <a href=\"https://help.clicksend.com/category/mfdctha7f0-country-specific-features-and-restrictions\" target=\"_blank\">Country Specific Features and Restrictions</a>. <div class=\"info-box\">   <h4><i class=\"fas fa-info-circle\"></i> Note :</h4>   ClickSend is pausing SMS messages containing URLs for new customers. Please remove links from messages to maintain uninterrupted delivery, or <a href=\"https://www.clicksend.com/gb/help/\" target=\"_blank\">contact support</a> to apply for approval for URL messaging. </div>

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package clicksend

import (
	"encoding/json"
)

// checks if the VerifyAllowedEmailAddressRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &VerifyAllowedEmailAddressRequest{}

// VerifyAllowedEmailAddressRequest struct for VerifyAllowedEmailAddressRequest
type VerifyAllowedEmailAddressRequest struct {
	To []SendEmailRequestToInner `json:"to,omitempty"`
	From *SendEmailVerificationTokenRequestFrom `json:"from,omitempty"`
	Subject *string `json:"subject,omitempty"`
	Body *string `json:"body,omitempty"`
}

// NewVerifyAllowedEmailAddressRequest instantiates a new VerifyAllowedEmailAddressRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewVerifyAllowedEmailAddressRequest() *VerifyAllowedEmailAddressRequest {
	this := VerifyAllowedEmailAddressRequest{}
	return &this
}

// NewVerifyAllowedEmailAddressRequestWithDefaults instantiates a new VerifyAllowedEmailAddressRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewVerifyAllowedEmailAddressRequestWithDefaults() *VerifyAllowedEmailAddressRequest {
	this := VerifyAllowedEmailAddressRequest{}
	return &this
}

// GetTo returns the To field value if set, zero value otherwise.
func (o *VerifyAllowedEmailAddressRequest) GetTo() []SendEmailRequestToInner {
	if o == nil || IsNil(o.To) {
		var ret []SendEmailRequestToInner
		return ret
	}
	return o.To
}

// GetToOk returns a tuple with the To field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VerifyAllowedEmailAddressRequest) GetToOk() ([]SendEmailRequestToInner, bool) {
	if o == nil || IsNil(o.To) {
		return nil, false
	}
	return o.To, true
}

// HasTo returns a boolean if a field has been set.
func (o *VerifyAllowedEmailAddressRequest) HasTo() bool {
	if o != nil && !IsNil(o.To) {
		return true
	}

	return false
}

// SetTo gets a reference to the given []SendEmailRequestToInner and assigns it to the To field.
func (o *VerifyAllowedEmailAddressRequest) SetTo(v []SendEmailRequestToInner) {
	o.To = v
}

// GetFrom returns the From field value if set, zero value otherwise.
func (o *VerifyAllowedEmailAddressRequest) GetFrom() SendEmailVerificationTokenRequestFrom {
	if o == nil || IsNil(o.From) {
		var ret SendEmailVerificationTokenRequestFrom
		return ret
	}
	return *o.From
}

// GetFromOk returns a tuple with the From field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VerifyAllowedEmailAddressRequest) GetFromOk() (*SendEmailVerificationTokenRequestFrom, bool) {
	if o == nil || IsNil(o.From) {
		return nil, false
	}
	return o.From, true
}

// HasFrom returns a boolean if a field has been set.
func (o *VerifyAllowedEmailAddressRequest) HasFrom() bool {
	if o != nil && !IsNil(o.From) {
		return true
	}

	return false
}

// SetFrom gets a reference to the given SendEmailVerificationTokenRequestFrom and assigns it to the From field.
func (o *VerifyAllowedEmailAddressRequest) SetFrom(v SendEmailVerificationTokenRequestFrom) {
	o.From = &v
}

// GetSubject returns the Subject field value if set, zero value otherwise.
func (o *VerifyAllowedEmailAddressRequest) GetSubject() string {
	if o == nil || IsNil(o.Subject) {
		var ret string
		return ret
	}
	return *o.Subject
}

// GetSubjectOk returns a tuple with the Subject field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VerifyAllowedEmailAddressRequest) GetSubjectOk() (*string, bool) {
	if o == nil || IsNil(o.Subject) {
		return nil, false
	}
	return o.Subject, true
}

// HasSubject returns a boolean if a field has been set.
func (o *VerifyAllowedEmailAddressRequest) HasSubject() bool {
	if o != nil && !IsNil(o.Subject) {
		return true
	}

	return false
}

// SetSubject gets a reference to the given string and assigns it to the Subject field.
func (o *VerifyAllowedEmailAddressRequest) SetSubject(v string) {
	o.Subject = &v
}

// GetBody returns the Body field value if set, zero value otherwise.
func (o *VerifyAllowedEmailAddressRequest) GetBody() string {
	if o == nil || IsNil(o.Body) {
		var ret string
		return ret
	}
	return *o.Body
}

// GetBodyOk returns a tuple with the Body field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VerifyAllowedEmailAddressRequest) GetBodyOk() (*string, bool) {
	if o == nil || IsNil(o.Body) {
		return nil, false
	}
	return o.Body, true
}

// HasBody returns a boolean if a field has been set.
func (o *VerifyAllowedEmailAddressRequest) HasBody() bool {
	if o != nil && !IsNil(o.Body) {
		return true
	}

	return false
}

// SetBody gets a reference to the given string and assigns it to the Body field.
func (o *VerifyAllowedEmailAddressRequest) SetBody(v string) {
	o.Body = &v
}

func (o VerifyAllowedEmailAddressRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o VerifyAllowedEmailAddressRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.To) {
		toSerialize["to"] = o.To
	}
	if !IsNil(o.From) {
		toSerialize["from"] = o.From
	}
	if !IsNil(o.Subject) {
		toSerialize["subject"] = o.Subject
	}
	if !IsNil(o.Body) {
		toSerialize["body"] = o.Body
	}
	return toSerialize, nil
}

type NullableVerifyAllowedEmailAddressRequest struct {
	value *VerifyAllowedEmailAddressRequest
	isSet bool
}

func (v NullableVerifyAllowedEmailAddressRequest) Get() *VerifyAllowedEmailAddressRequest {
	return v.value
}

func (v *NullableVerifyAllowedEmailAddressRequest) Set(val *VerifyAllowedEmailAddressRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableVerifyAllowedEmailAddressRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableVerifyAllowedEmailAddressRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVerifyAllowedEmailAddressRequest(val *VerifyAllowedEmailAddressRequest) *NullableVerifyAllowedEmailAddressRequest {
	return &NullableVerifyAllowedEmailAddressRequest{value: val, isSet: true}
}

func (v NullableVerifyAllowedEmailAddressRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVerifyAllowedEmailAddressRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


