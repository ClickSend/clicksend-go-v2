/*
SMS

You can use the SMS endpoints to send and manage SMS, check the cost of sending SMS, view message history, and more. Integrate them into your system as part of your workflow.  Relevant guides: <a href=\"https://help.clicksend.com/article/h474eseq3a-how-many-characters-can-i-send-in-an-sms\" target=\"_blank\">How many characters can I send in an SMS?</a>, <a href=\"https://help.clicksend.com/article/1hd33n2xns-sms-status-delivery-receipt-expired\" target=\"_blank\">SMS status delivery receipt expired</a>, <a href=\"https://help.clicksend.com/en/articles/42195-unicode-vs-plain-text\" target=\"_blank\">Do you support non-English characters</a>, and <a href=\"https://help.clicksend.com/category/mfdctha7f0-country-specific-features-and-restrictions\" target=\"_blank\">Country Specific Features and Restrictions</a>. <div class=\"info-box\">   <h4><i class=\"fas fa-info-circle\"></i> Note :</h4>   ClickSend is pausing SMS messages containing URLs for new customers. Please remove links from messages to maintain uninterrupted delivery, or <a href=\"https://www.clicksend.com/gb/help/\" target=\"_blank\">contact support</a> to apply for approval for URL messaging. </div>

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package clicksend

import (
	"encoding/json"
)

// checks if the SendLetterData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SendLetterData{}

// SendLetterData struct for SendLetterData
type SendLetterData struct {
	Recipients []Recipient `json:"recipients,omitempty"`
	Currency *Currency `json:"_currency,omitempty"`
}

// NewSendLetterData instantiates a new SendLetterData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSendLetterData() *SendLetterData {
	this := SendLetterData{}
	return &this
}

// NewSendLetterDataWithDefaults instantiates a new SendLetterData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSendLetterDataWithDefaults() *SendLetterData {
	this := SendLetterData{}
	return &this
}

// GetRecipients returns the Recipients field value if set, zero value otherwise.
func (o *SendLetterData) GetRecipients() []Recipient {
	if o == nil || IsNil(o.Recipients) {
		var ret []Recipient
		return ret
	}
	return o.Recipients
}

// GetRecipientsOk returns a tuple with the Recipients field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SendLetterData) GetRecipientsOk() ([]Recipient, bool) {
	if o == nil || IsNil(o.Recipients) {
		return nil, false
	}
	return o.Recipients, true
}

// HasRecipients returns a boolean if a field has been set.
func (o *SendLetterData) HasRecipients() bool {
	if o != nil && !IsNil(o.Recipients) {
		return true
	}

	return false
}

// SetRecipients gets a reference to the given []Recipient and assigns it to the Recipients field.
func (o *SendLetterData) SetRecipients(v []Recipient) {
	o.Recipients = v
}

// GetCurrency returns the Currency field value if set, zero value otherwise.
func (o *SendLetterData) GetCurrency() Currency {
	if o == nil || IsNil(o.Currency) {
		var ret Currency
		return ret
	}
	return *o.Currency
}

// GetCurrencyOk returns a tuple with the Currency field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SendLetterData) GetCurrencyOk() (*Currency, bool) {
	if o == nil || IsNil(o.Currency) {
		return nil, false
	}
	return o.Currency, true
}

// HasCurrency returns a boolean if a field has been set.
func (o *SendLetterData) HasCurrency() bool {
	if o != nil && !IsNil(o.Currency) {
		return true
	}

	return false
}

// SetCurrency gets a reference to the given Currency and assigns it to the Currency field.
func (o *SendLetterData) SetCurrency(v Currency) {
	o.Currency = &v
}

func (o SendLetterData) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SendLetterData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Recipients) {
		toSerialize["recipients"] = o.Recipients
	}
	if !IsNil(o.Currency) {
		toSerialize["_currency"] = o.Currency
	}
	return toSerialize, nil
}

type NullableSendLetterData struct {
	value *SendLetterData
	isSet bool
}

func (v NullableSendLetterData) Get() *SendLetterData {
	return v.value
}

func (v *NullableSendLetterData) Set(val *SendLetterData) {
	v.value = val
	v.isSet = true
}

func (v NullableSendLetterData) IsSet() bool {
	return v.isSet
}

func (v *NullableSendLetterData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSendLetterData(val *SendLetterData) *NullableSendLetterData {
	return &NullableSendLetterData{value: val, isSet: true}
}

func (v NullableSendLetterData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSendLetterData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


